...
export class UserController {
    ...
    static sendEmailVerification(req: Request, res: Response) {
        ...
        if (!email) {
            ...
            return Nodemailer.sendEmail('shagungarg2010@gmail.com', email, 'email verification', html)
                .then((data) => {
                    return res.status(201).send(data)
                }).catch(err => {
                    return res.status(500).send(err)
    ...
    static sendResetPasswordMail(req: Request, res: Response) {
        ...
        if (!email) {
            ...
            User.findOne({email: email}).then((data) => {
                ...
                Nodemailer.sendEmail(' Digi Resume <contact@digiresumes.com>', email, 'Password Reset Email', html).then(() => {
                    res.status(200).json({
